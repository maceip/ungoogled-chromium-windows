...
# Iterate through whatever version folders actually exist
for d in Windows?Kits/10/Include/*; do
    test -d "$d" || continue
    case_fold_dir "$d/shared"
    # Try the windows subfolder if it exists, but don't fail if it doesn't
    if [ -d "$d/shared/windows" ]; then
        case_fold_dir "$d/shared/windows"
    fi
    case_fold_dir "$d/um"
    case_fold_dir "$d/winrt"
done
...
for d in Windows?Kits/10/Include/*; do
    # Only run if the directory actually exists to avoid "not found" errors
    if [ -d "$d/shared" ]; then
        mixed_case_files "$d/shared" \
            DXGI1_4.h DXGIType.h DriverSpecs.h POPPACK.H PSHPACK1.H \
            Sddl.h SpecStrings.h WinDef.h WlanTypes.h
    fi
done

# 2. Process um headers
for d in Windows?Kits/10/Include/*; do
	if [ -d "$d/um" ]; then
		mixed_case_files "$d/um" \
			AudioClient.h \
			D3D11.h \
			D3D11_1.h \
			D3D11_2.h \
...
do
	for dir in Windows?Kits/10/Lib/*/um/$arch
	do
		test -d "$dir" || continue
